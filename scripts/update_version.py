...
Usage:
    python scripts/update_version.py <gopher_orch_version>
...
This updates:
    ...
    - gopher_orch/__init__.py version
...
def parse_version(gopher_orch_version: str) -> tuple[str, str]:
    """
    Convert gopher-orch version tag to Python PEP 440 version.

    Args:
        gopher_orch_version: e.g., "v0.1.0-20260225-132506" or "0.1.0-20260225-132506"

    Returns:
        tuple of (gopher_orch_tag, python_version)
        e.g., ("v0.1.0-20260225-132506", "0.1.0.dev20260225132506")
    """
    # Remove 'v' prefix if present
    version = gopher_orch_version.lstrip('v')
    ...
    gopher_orch_tag = f"v{version}" if not gopher_orch_version.startswith('v') else gopher_orch_version

    return gopher_orch_tag, python_version
...
def update_workflow(file_path: Path, gopher_orch_tag: str, python_version: str) -> None:
    ...
    # Update GOPHER_ORCH_VERSION
    content = re.sub(
        r'GOPHER_ORCH_VERSION:\s*[^\n]+',
        f'GOPHER_ORCH_VERSION: {gopher_orch_tag}',
        content
    )
...
def update_main_package(python_version: str) -> None:
    ...
    update_init_py(ROOT_DIR / "gopher_orch" / "__init__.py", python_version)
...
def update_platform_packages(python_version: str) -> None:
    ...
    for platform in PLATFORMS:
        ...
        pkg_name = f"gopher_orch_native_{platform.replace('-', '_')}"
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scripts/update_version.py <gopher_orch_version>")
        print("Example: python scripts/update_version.py v0.1.0-20260225-132506")
        sys.exit(1)

    gopher_orch_version = sys.argv[1]
    gopher_orch_tag, python_version = parse_version(gopher_orch_version)

    print(f"\nUpdating versions:")
    print(f"  gopher-orch tag: {gopher_orch_tag}")
    ...
    update_workflow(workflow_path, gopher_orch_tag, python_version)
