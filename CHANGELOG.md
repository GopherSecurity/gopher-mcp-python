...
- Preflight provider OAuth elicitation for any HTTP(S) MCP server URL used with
  `GopherAgent.create_with_url`, not only gateway-shaped URLs.
- Pass accepted preflight MCP session state into native agent creation with
  internal coordination headers so discovery and tool calls reuse the same
  authorized backend session.
- Add regression coverage for generic MCP URL preflight, accepted session
  propagation, and `create_with_url` native runtime option wiring.

### Changed

- Pin `gopher-orch` native library to [v0.1.39](https://github.com/GopherSecurity/gopher-orch/releases/tag/v0.1.39).
- Track the vendored `gopher-orch` submodule on `br_release`.
- Run the `examples/api/create_by_url_run.sh` example against the local Python
  checkout and locally built native libraries for development verification.

### Fixed

- Fix live-server `create_with_url` provider OAuth flows that could ask for an
  additional authorization after the initial preflight had already completed.
  The SDK now keeps the accepted session aligned with the native Streamable HTTP
  client instead of allowing discovery/tool-call sessions to diverge.
- Avoid sending the preflight `Mcp-Session-Id` as a public runtime header; it is
  now used only for internal SDK/native coordination.

#### SDK changes since v0.1.34

- Fix create_with_url elicitation session reuse (#21)
...
#### gopher-orch v0.1.39 highlights
...
### Fixed
- Fix streamable HTTP elicitation session reuse (https://github.com/GopherSecurity/gopher-orch/pull/188)
- Fix gateway build push without cache (https://github.com/GopherSecurity/gopher-orch/pull/186)
