...
class TestTokenPayloadExtendedFields:
    ...
    def test_name_field(self):
        payload = TokenPayload(
            subject="user-123", scopes="openid", name="John Doe"
        )
    ...
    def test_server_id_field(self):
        payload = TokenPayload(
            subject="user-123", scopes="openid", server_id="srv-789"
        )
...
class TestGopherAuthContextExtendedFields:
    ...
    def test_email_field(self):
        ctx = GopherAuthContext(
            user_id="u", scopes="s", audience="a",
            token_expiry=0, authenticated=True, email="user@example.com"
        )
    ...
    def test_name_field(self):
        ctx = GopherAuthContext(
            user_id="u", scopes="s", audience="a",
            token_expiry=0, authenticated=True, name="John"
        )
    ...
    def test_organization_id_field(self):
        ctx = GopherAuthContext(
            user_id="u", scopes="s", audience="a",
            token_expiry=0, authenticated=True, organization_id="org-1"
        )
    ...
    def test_server_id_field(self):
        ctx = GopherAuthContext(
            user_id="u", scopes="s", audience="a",
            token_expiry=0, authenticated=True, server_id="srv-1"
        )
    ...
    def test_raw_token_field(self):
        ctx = GopherAuthContext(
            user_id="u", scopes="s", audience="a",
            token_expiry=0, authenticated=True,
